...
public class SmartHealthcareSimulation {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Faulty VM simulation
            int faultyVmIndex = new Random().nextInt(vmCount);
            System.out.println("⚠️ Simulating VM Failure on VM ID: " + faultyVmIndex);

            int roundRobinIndex = 0;
            for (Cloudlet cloudlet : cloudletList) {
                while (roundRobinIndex == faultyVmIndex) {
                    roundRobinIndex = (roundRobinIndex + 1) % vmCount;
                }
                cloudlet.setVmId(vmList.get(roundRobinIndex).getId());
                roundRobinIndex = (roundRobinIndex + 1) % vmCount;
            }
